...
import { Grid, InputGroup, Input, InputLeftAddon, Text, Accordion, AccordionItem, AccordionPanel, AccordionHeader, AccordionIcon, Box, Heading, Select, Checkbox } from "@chakra-ui/core";
...
export default function Projects() {
	...
	async function handleSubmitNewProject(event) {
		...
		try {
			...
			if (data.id) {
				setMessage(`Projeto ${name} (${description}) criado com sucesso.\nID do projeto: ${data.id}\nSlug do projeto: ${data.slug}`);
	...
	function handleSelectProject(event) {
		...
		const projectToSelect = projects.find((project) => project.id == projectIdToFind);
	...
	async function handleUpdateProject(event) {
		...
		try {
			...
			if (data.message !== "Nenhuma alteração a ser salva neste projeto.") setRefreshProjectsContext(!refreshProjectsContext);
		} catch (error) {
	...
	useEffect(() => {
		const projectToSelect = projects.find((project) => project.id == selectedProject?.id);
		if (projectToSelect) {
		...
		<>
			<Accordion>
				<AccordionItem>
					...
					<AccordionPanel pb={0}>
						<Grid as='form' margin='64px auto' gap='16px' maxWidth='480px' onSubmit={handleSubmitNewProject}>
							<ProjectFields />
							<Input type='submit' backgroundColor='blue.500' _hover={{ backgroundColor: "blue.600" }} value='CADASTRAR' maxWidth='50%' margin='0 auto' />
						</Grid>
					</AccordionPanel>
				</AccordionItem>
				{selectedProject && (
					<AccordionItem>
						...
						<AccordionPanel pb={0}>
							<Grid as='form' margin='16px auto' gap='16px' onSubmit={handleUpdateProject}>
								<InputGroup>
									<InputLeftAddon children='Projeto' />
									<Select variant='filled' color='gray.600' name='projectID' onChange={handleSelectProject} isRequired>
										{projects.map((project, index) => {
											return (
												<option value={project.id} key={index} selected={selectedProject.id == project.id}>
													{project.name}
												</option>
											);
										})}
								...
								<ProjectFields
									...
									update={({ name, description, trackerFacebook, trackerGoogleAnalytics, trackerGoogleAds, team }) => {
										setSelectedProject({
											...selectedProject,
											name,
											description,
								...
								<Input type='submit' backgroundColor='blue.500' _hover={{ backgroundColor: "blue.600" }} value='SALVAR' maxWidth='50%' margin='0 auto' />
							</Grid>
						</AccordionPanel>
					</AccordionItem>
				)}
